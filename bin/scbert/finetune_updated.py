...
parser.add_argument("--eval_size", type=float, default=0.2, help="Size of dataset for evaluating the model")
    ...
    if i % VALIDATE_EVERY == 0:
        ...
        with torch.no_grad():
            ...
            if is_master:
                ...
                print(classification_report(truths, predictions, labels = list(range(len(label_dict))), target_names=label_dict.tolist(), digits=4))
            if cur_acc > max_acc:
                max_acc = cur_acc
                trigger_times = 0
...
label2id = {item:i for i,item in enumerate(label_dict)}
