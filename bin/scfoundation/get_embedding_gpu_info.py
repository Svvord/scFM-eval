...
parser.add_argument('--task_name', type=str, default='zeroshot', help='task name')
...
## update by Siyu
gene_list_df = pd.read_csv(os.path.join(os.path.dirname(__file__), 'OS_scRNA_gene_index.19264.tsv'), header=0, delimiter='\t')
## end update
...
def main():
    ...
    # pca_coords = geneexpemb
    # var_names = [f'V{i+1}' for i in range(pca_coords.shape[1])]
    # adata_pca = sc.AnnData(X=pca_coords, obs=adata.obs.copy(), var=pd.DataFrame(index=var_names))
    # adata_pca.obsm['spatial'] = adata.obsm['spatial']
    # adata_pca.write("embedding.h5ad")
    ## end update
